# SignalX monaco-editor — shared agent guide
...
SignalX `monaco-editor` (`signalxjs/monaco-editor`) — Monorepo for @sigx/monaco-editor — a pluggable Monaco wrapper for sigx. A pnpm monorepo (ESM, `"type": "module"`) with the package under `packages/`. Tech stack: TypeScript (strict), Vitest, oxlint; published to npm under the `@sigx` scope.
...
Never commit straight to `main`.** Repo: `signalxjs/monaco-editor`, base branch `main`.
...
4. **Open a PR with Copilot as the reviewer.** Reference the issue so it auto-closes
   ...
   ```sh
   gh api --method POST repos/signalxjs/monaco-editor/pulls/<pr>/requested_reviewers \
...
5. **Wait for Copilot's review, then fix.** Do not merge before it has reviewed. Poll
   ...
   **Then resolve the threads.** The sigx-standard ruleset sets
   ...
   gh api graphql -f query='query { repository(owner:"signalxjs", name:"monaco-editor") {
...
- `packages/monaco-editor` → `@sigx/monaco-editor` — Pluggable Monaco editor wrapper for sigx with prebundled assets and a Vite plugin that bypasses bundler optimization.
...
- **Before merging a PR with user-facing changes, file an issue on the docs
  ...
  gh issue create --repo signalxjs/signalxjs.github.io \
    --title "monaco-editor: <what changed>" \
    --body "Source: signalxjs/monaco-editor#<pr>. <What needs documenting, and where on the site.> Not yet released."
  ```
...
- **When you cut a release** (push a `vX.Y.Z` tag), comment the release tag on
  ...
  gh issue comment <n> --repo signalxjs/signalxjs.github.io \
    --body "Released in monaco-editor vX.Y.Z."
  ```
...
4. Adapt the repo-specific sections of `AGENTS.md`: the intro (what the repo is),
   "Build, Test, Lint", and "Packages". Replace every `monaco-editor` with the repo name.
5. Keep the workflow, worktree, and conventions sections as-is — they are the
   shared standard.
6. Lock down `main`: `node scripts/apply-branch-protection.mjs signalxjs/monaco-editor`.
